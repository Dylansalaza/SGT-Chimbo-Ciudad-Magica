...
<body class="app-bg antialiased min-h-screen flex items-center justify-center p-4">
    ...
    <div class="card-in card-shadow w-full max-w-md bg-white rounded-3xl overflow-hidden">
        ...
        <div class="p-8">
            ...
            {{-- Confirmación de envío. La contraseña SOLO se restablece con el
                 enlace que llega al correo (el token nunca se muestra aquí).
                 Si el correo no está registrado, se avisa en el bloque de error. --}}
            @if(session('status'))
                <div class="bg-green-50 border border-green-300 text-green-900 px-4 py-4 rounded-xl mb-5 text-sm flex items-start gap-2">
            ...
            @if(session('error'))
                <div class="bg-red-50 border border-red-300 text-red-800 px-4 py-3 rounded-xl mb-4 text-sm font-semibold flex items-center gap-1.5">
