...
<div class="w-full flex flex-col">
    ...
    <div class="sticky top-0 z-50 bg-[#00294d] text-white w-full px-10 py-8 shadow-lg border-b border-white/5">
        <div class="w-full flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6">
            ...
            <a href="{{ route('admin.noticias.create') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black tracking-wider shadow-md transition-all uppercase self-start sm:self-center">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="hover:bg-slate-50/40 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm font-semibold">
                                @if($n->published_at)
                                    <span class="text-emerald-600">{{ date('d/m/Y', strtotime($n->published_at)) }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 text-sm text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.noticias.edit', $n->id) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100/70 transition">
